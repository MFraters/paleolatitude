...
public:
	struct Entry {
	public:
		virtual void set(unsigned int column, const string value, unsigned int lineno) = 0;
	...
	public:
		virtual void set(unsigned int column, const string value, unsigned int lineno){
	...
	void parseStream(istream* csvfile){
		string line;
		...
		while (getline(*csvfile, line)){
			...
			if (values.size() != csv_entry.numColumns()){
				if (lines_parsed > 0 || line_no >= 3){
					...
					CSVFileDataParseException ex;
					ex << "Parse error on line " << line_no << " of csv file: expecting " << csv_entry.numColumns() << " values, got " << values.size();
					throw ex;
			...
			try {
				for (unsigned int i = 0; i < values.size(); i++){
					csv_entry.set(i, values[i], line_no);
				}
		...
		if (lines_parsed == 0){
			throw CSVFileDataParseException("Could not parse information from csv file: no valid comma-separated lines found");
		}
	}
