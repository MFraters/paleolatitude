...
#include "paleo_latitude/PaleoLatitudes.h"
...
int main(int argc, char* argv[]) {
	...
	cmdline_params_spec.add_options()
		...
		("input-list-csv-file", bpo::value<string>(&pl_params->input_list_csv_file), "path to csv input list file or string containing the csv.")
		("input-list-csv-is-file", bpo::value<bool>(&pl_params->input_list_csv_is_file), "sets wether the input list csv file is interpeted as file name (true) or as a string containing the csv data iself (false).")
	...
	if (cmdline_params_values.count("input-list-csv-is-file") > 0){
		// A whole csv file needs to be processed
		PaleoLatitudes* pls = NULL;
        //std::cin >> pl_params->input_list_csv_file;

		try {
			pls = new PaleoLatitudes(pl_params->input_list_csv_file, pl_params->input_list_csv_is_file);

			if(!pls->compute(pl_params)){
			...
			cerr << "Unexpected error computing paleolatitudes: " << ex.what() << endl;
			exit(1);
		}
		...
		//pls->writeCSV(cout);
			...
			// Export CSV
			try{
				pls->writeCSV(cmdline_params_values["csv-output-file"].as<string>());
			} catch (exception& ex){
	...
	{
		// Only one Paleolatitude is needed 
