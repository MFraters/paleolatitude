 ...
 *      Author: Sebastiaan J. van Schaik, Menno R.T. Fraters
 *
 *
...
void PaleoLatitude::printAbout() {
	...
	cout << "Implementation by Sebastiaan J. van Schaik (sjvs@traiectum.net) and Large dataset handling implemented by Menno R. T. Fraters" << endl;
...
PaleoLatitude::PaleoLatitudeEntry PaleoLatitude::getPaleoLatitude() const {
	...
	for (PaleoLatitudeEntry entry : _result){
		if (aggr.age_years == 99999){
			...
			aggr.palat_min = 90;
			aggr.palat_max = -90;
			aggr.age_years_lower_bound = std::numeric_limits<long>::max();
			aggr.age_years_upper_bound = 0;
		}

		if(entry.age_years <= _params->getMaxAgeInYears() && entry.age_years >= _params->getMinAgeInYears())
		{
